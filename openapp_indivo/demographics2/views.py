...
def demographics(request, sessionkey):
    """
        This is a version of the demographics display which uses the 
        indivo database directly. It has less code.
    ...
    if record_id:
        record = indivo.models.Record.objects.get(id=record_id)
        isrecord = True
    else:
        carenet_id = session['carenet_id']
        isrecord = False
        record = indivo.models.Carenet.objects.get(id=carenet_id).record

    return render_to_response("openapp_indivo/demographics/demographics.html",
        {'demographics': record.demographics, 'isrecord': isrecord},
